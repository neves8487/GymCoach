...
if _PT_URL or _NUTRI_URL:
    # A2A Remote Mode — wrap RemoteA2aAgent in AgentTool for tool-based delegation
    from google.adk.agents.remote_a2a_agent import RemoteA2aAgent

    if _PT_URL:
        _pt_remote = RemoteA2aAgent(
            name="pt_agent",
            description=(
        ...
        _tools.insert(0, AgentTool(agent=_pt_remote))

    if _NUTRI_URL:
        _nutri_remote = RemoteA2aAgent(
        ...
        _tools.insert(1, AgentTool(agent=_nutri_remote))
else:
    # Local Mode — delegate in-process via AgentTool
...
root_agent = Agent(
    ...
    description=(
        ...
        "e nutrição, gere perfis de utilizador, e interage via WhatsApp/Telegram."
    ),
